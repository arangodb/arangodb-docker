...
while (true) {
  result = download("http://agency:4001/v2/keys/arango/Current/NewServers","",{"method":"GET"});
  if (result.code !== 200) {
  ...
  result = download("http://agency:4001/v2/keys/arango/Target/MapLocalToID","",{"method":"GET"});
  if (result.code !== 200) {
    print("Received error from agency: ", result);
    break;
  }
  ...
  for (i = 0; i < newservers.length; i++) {
    if (oldservers.indexOf(newservers[i]) === -1) {
      ...
      var url = "http://agency:4001/v2/keys/arango/Plan/" + entity + "/" +
                serverName;
      ...
      url = "http://agency:4001/v2/keys/arango/Target/MapLocalToID/"+newservers[i];
